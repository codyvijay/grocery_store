export default{
    ...
    <h2 class="border-bottom p-4 d-flex justify-content-center">Products in {{cat_name}}</h2>
    ...
    data(){
        return {
            cat_name:"", cat_code:"",products:[], ploading:true, error:false, error_message:"",
        }
    ...
    created: function(){
        this.cat_code=this.$route.params.id;
        this.cat_name=this.$route.params.name;
        fetch('/api/product', {
            method:'GET',
            headers:{
            ...
            for(let i=0; i<data.length; i++){
                if(data[i].section_code==this.cat_code){
